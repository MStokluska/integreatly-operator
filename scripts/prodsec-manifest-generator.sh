...
"production")
    case $OLM_TYPE in
      "integreatly-operator")
        OLM_TYPE="rhmi"
        VERSION=$(grep integreatly-operator.v deploy/olm-catalog/integreatly-operator/integreatly-operator.package.yaml | tail -c 6)
        FILE_NAME="rhmi-production-release-manifest.txt"
        SERVICE_NAME="services-rhmi"
        ;;
      "managed-api-service")
        OLM_TYPE="rhoam"
        VERSION=$(grep managed-api-service.v deploy/olm-catalog/managed-api-service/managed-api-service.package.yaml | tail -c 6)
